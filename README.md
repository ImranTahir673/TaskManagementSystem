...
Backend Ecosystem: .NET 8.0 SDK, Entity Framework Core 8.0, Microsoft.EntityFrameworkCore.SqlServer (fully configured for enterprise LocalDB schema execution testing).

Frontend Ecosystem: React 18, TypeScript, Tailwind CSS Framework, Axios Interceptors (optimized for dynamic client-side JWT authorization management).
...
1. Regular Users
Strict data isolation boundary. Restricted from administrative metrics; can exclusively view and manage metrics solely for tasks assigned directly to their unique workspace profile identifier.

Self-assignment tracking with robust delegation restrictions.

2. Administrators (Executive Controls)
Global project health visibility tracking system counters across all backend database metrics.
...
This repository is configured with static code analysis scanning pipelines to enforce defensive coding architectures.
...
If you prefer to compile, run, and evaluate the environmental structures locally on your host machine, execute these steps sequentially within separate terminal windows:

Prerequisites
Ensure your development environment has .NET 8.0 SDK and Node.js (v18+ or v20+) installed.
